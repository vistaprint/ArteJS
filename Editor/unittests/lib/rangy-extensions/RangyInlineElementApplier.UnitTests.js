$(document).ready(function ()
{
    ...
    {
        $.extend(testData.options, { topEditableParent: $(TEST_ELEMENT_SELECTOR).find("[contenteditable=true]").get(0) });
...
var inlineOptions = { applierTagName: "span", commandAttrType: $.Arte.constants.commandAttrType.styleName };
var inlineOptionsWithFontWeight = $.extend({}, inlineOptions, { styleName: "font-weight", styleValue: "bold" });
var inlineOptionsWithBTag = $.extend({}, inlineOptions, { applierTagName: $.Arte.constants.tagName.B, tagName: $.Arte.constants.tagName.B, commandAttrType: $.Arte.constants.commandAttrType.tagName });
var inlineOptionsWithClassName = $.extend({}, inlineOptions, { className: "arte-font-weight-bold", commandAttrType: $.Arte.constants.commandAttrType.className});

var inlineOptionsWithFontStyle = $.extend({}, inlineOptions, { styleName: "font-style", styleValue: "italic" });
var inlineOptionsWithColor = $.extend({}, inlineOptions, { styleName: "color", styleValue: "red" });

var inlineOptionsWithFontSize = $.extend({}, inlineOptions, { styleName: "font-size", styleValue: "12px" });

var inlineOptionsWithFontFamily = $.extend({}, inlineOptions, { styleName: "font-family", styleValue: "georgia" });
...
function complexInlineCommandUnitTests()
{
	...
	test(complexInlineCommandsUnitTestData.splitBoldNode.name, function ()
	{
		...
		{
			var options = $.extend({}, testData.options, { topEditableParent: $("#editableDiv").get(0) });
	...
	test(complexInlineCommandsUnitTestData.splitBoldItalicNode.name, function ()
	{
		...
		unitTestHelper.executeTest(testData, function ()
		{
			...
			rangy.toggleStyleOnSelection($.extend({}, inlineOptionsWithFontStyle, { topEditableParent: $("#editableDiv").get(0) }));
			$.Arte.dom.cleanup($('[contenteditable="true"]'));
	...
	test(complexInlineCommandsUnitTestData.changeFontFamilyOfSelection.name, function ()
	{
	    ...
	    unitTestHelper.executeTest(testData, function ()
	    {
	        var options = $.extend({}, inlineOptionsWithFontFamily, { topEditableParent: $("#editableDiv").get(0) });
	        ...
	        unitTestHelper.createSelection(testData);
	        rangy.toggleStyleOnSelection($.extend({}, options, { styleValue: "arial" }));
	        $.Arte.dom.cleanup($('[contenteditable="true"]'));
	    });
	});
