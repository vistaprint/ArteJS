...
(function($)
{
    ...
    $.extend($.expr[":"], {
        ...
        * Usage: $("body :block") - returns all block elements inside body tag. $(element).is(":block") will check if element is block or not.
        */
    ...
    */
    ...
    $.extend(dom, {
        ...
        closestWithAtMostOneBlockChild: function(jNode, topMostElement)
        {
            ...
            jNode.each(function()
            {
                ...
                $(this).parentsUntil(topMostElement).each(function()
                {
                    ...
                    var isValid = (blockChildrenCount === 0) || // 0 or 1 block child
                        (blockChildrenCount === 1 && blockChildrenCount === this.childNodes.length); // There are not other non-block children
                    if (isValid)
                    {
                        parent = this;
                    }
        ...
        */
        ...
        {
            var returnValue = {};
            if (jNode.prop("nodeType") === constants.nodeType.TEXT)
            { // Text nodes won't have any styles set.
                return returnValue;
            }
        ...
        */
        ...
        {
            return dom.closestWithCommand(jNode, $.extend(options, { checkValue: 1 }));
        },
        ...
        */
        listSurrounded: function(jNode, options)
        {
            ...
            if (allNodesListSurrounded)
            {
                ...
                { // the LI elements belong to different lists
                    return false;
                }
...
rangy.createModule("InlineElementApplier", function(api)
{
    ...
    */
    function applyToTextNode(textNode, options)
    {
        ...
        if (children.length > 1 || !configuration.styleableTags[parent.prop("tagName")] || options.commandAttrType === constants.commandAttrType.tagName)
        {
    ...
    */
    ...
    {
        var parentWithStyle = dom.closestWithCommand(textNode, $.extend(options, { checkValue: true }));
        ...
        if (options.styleName && dom.hasCommandValue(parentWithStyle, { styleName: options.styleName, styleValue: options.styleValue }))
        {
        ...
        else if (options.className && dom.hasCommandValue(parentWithStyle, { className: options.className }))
        {
...
(function($)
{
    ...
    $.Arte.RichTextCommandApplier = function(options)
    {
        ...
        var applyToTextNodes = function(commandInfo, type)
        {
            ...
            var commandOptions = { topEditableParent: contentEditableContainer };
            ...
            switch (type)
            {
                case commandType.inline:
                    ...
                    {
                        return range ? rangy.toggleStyleOnRange(range, commandOptions) : rangy.toggleStyleOnSelection(commandOptions);
                    };
                    break;
                case commandType.block:
                    commandToExecute = function()
                    {
                        return range ? rangy.toggleSurroundRange(range, commandOptions) : rangy.toggleSurroundSelection(commandOptions, contentEditableContainer);
                    };
                    break;
                case commandType.complex:
                    commandToExecute = function()
                    {
                        return range ? rangy.toggleSurroundRangeSet(range, commandOptions) : rangy.toggleSurroundSelectionSet(commandOptions, contentEditableContainer);
                    };
                    break;
            }
    ...
    var undoManager = function()
    {
        return {
            ...
            */
            ...
            {
                textArea.undoInfo = textArea.undoInfo || { stack: [], index: -1 };
